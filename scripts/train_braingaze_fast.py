"""
train_braingaze_fast.py
========================
Minimal-epoch training for BrainGaze v3 — just enough to prove EEG bypass is fixed.

  3 warmup + 7 joint = 10 epochs total

Simplifications vs. full v3 training:
  - No MMPareto gradient alignment (overhead)
  - Aggressive noise decay (5 epochs, not 25)
  - Higher learning rates for faster convergence
  - Same 3-pass joint training (main + discrimination + aux) — these are essential
"""
...
def train():
    CKPT_DIR = os.path.join(PROJECT_ROOT, "outputs", "models", "checkpoints_v3")
    PROG_DIR = os.path.join(PROJECT_ROOT, "outputs", "epoch_progression_v3")
    ...
    # Model — aggressive noise decay (5 joint epochs)
    model = BrainGazeDiffusionModel(
        noise_sigma=0.5, noise_decay_epochs=5
    ).to(DEVICE)
    ...
    print(f"\n{'='*60}")
    print(f"  Fast Training: {EPOCHS} epochs ({WARMUP_EPOCHS}W + {EPOCHS-WARMUP_EPOCHS}J)")
    ...
    for epoch in range(EPOCHS):
        ...
        torch.save(model.state_dict(),
                   os.path.join(CKPT_DIR, f"braingaze_v3_epoch_{epoch+1}.pth"))
    ...
    print(f"  Now run:  python scripts/run_diagnostics_v3.py")
    print(f"{'='*60}")
